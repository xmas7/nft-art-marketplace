...
abstract contract NFTMarketBuyPrice is NFTMarketCore, NFTMarketFees {
   ...
   * @dev The total buy price that was accepted is `museeFee` + `creatorFee` + `ownerRev`.
  ...
  function setBuyPrice(
    ...
    if (price < 0.01 ether) {
      ...
      revert NFTMarketBuyPrice_Too_Low_Value_Provided();
    }
  ...
  function _buy(address nftContract, uint256 tokenId) private nonReentrant {
    ...
    emit BuyPriceAccepted(nftContract, tokenId, buyPrice.seller, msg.sender, museeFee, creatorFee, ownerRev);
  }
...
/*╔═════════════════════════════════════════════════════════╗
  ║        BUY NOW Functionality is implemented here!       ║
  ╚═════════════════════════════════════════════════════════╝*/