...
error NFTMarketReserveAuction_Too_Low_Value_Provided();
  ...
  NFTMarketAuction
{
  ...
  /**
   * @notice Emitted when an auction is canceled by a Musee admin.
   ...
   * @dev The amount of the highest bid / final sale price for this auction is `museeFee` + `creatorFee` + `ownerRev`.
   * @param auctionId The id of the auction that was finalized.
   * @param seller The address of the seller.
   * @param bidder The address of the highest bidder that won the NFT.
   * @param museeFee The amount of ETH that was sent to Musee for this sale.
  ...
  modifier onlyValidAuctionConfig(uint256 reservePrice) {
    ...
    if (reservePrice < 0.01 ether) {
      // min start price - 0.01 ETH
      revert NFTMarketReserveAuction_Too_Low_Value_Provided();
    }
    _;
  }
  ...
  /**
   * @notice Allows Musee to cancel an auction, refunding the bidder and returning the NFT to
  ...
  function _finalizeReserveAuction(uint256 auctionId, bool keepInEscrow) private {
    ...
    (uint256 museeFee, uint256 creatorFee, uint256 ownerRev) = _distributeFunds(
      auction.nftContract,
      auction.tokenId,
      auction.seller,
      auction.amount
    ...
    emit ReserveAuctionFinalized(auctionId, auction.seller, auction.bidder, museeFee, creatorFee, ownerRev);
  }
...
/*╔══════════════════════════════════════════════════════════════════════════════╗
  ║        Functionalities related to RESERVE AUCTION is implemented here!       ║
  ╚══════════════════════════════════════════════════════════════════════════════╝*/