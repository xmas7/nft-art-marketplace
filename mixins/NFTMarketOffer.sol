 ...
 */
abstract contract NFTMarketOffer is MuseeTreasuryNode, NFTMarketCore, ReentrancyGuardUpgradeable, NFTMarketFees {
   ...
   * @dev The accepted total offer amount is `museeFee` + `creatorFee` + `ownerRev`.
  ...
  /**
   * @notice Emitted when an offer is canceled by a Musee admin.
  ...
  /**
   * @notice Allows Musee to cancel offers.
  ...
  ) external onlyMuseeAdmin nonReentrant {
  ...
  function _acceptOffer(address nftContract, uint256 tokenId) private {
    ...
    emit OfferAccepted(nftContract, tokenId, offer.buyer, msg.sender, museeFee, creatorFee, ownerRev);
  }
