...
abstract contract NFTMarketPrivateSale is NFTMarketFees {
  ...
  string private constant NAME = "MUSEENFTMarket";

  /**
   * @notice Emitted when an NFT is sold in a private sale.
   * @dev The total amount of this sale is `museeFee` + `creatorFee` + `ownerRev`.
   ...
   * withdrawn from your METH balance.
   * @param deadline The timestamp at which the offer to sell will expire.
   * @param v The v value of the EIP-712 signature.
   * @param r The r value of the EIP-712 signature.
   * @param s The s value of the EIP-712 signature.
  ...
  function buyFromPrivateSaleFor(
    ...
    (uint256 museeFee, uint256 creatorFee, uint256 ownerRev) = _distributeFunds(nftContract, tokenId, seller, amount);

    emit PrivateSaleFinalized(nftContract, tokenId, seller, msg.sender, museeFee, creatorFee, ownerRev, deadline);
  }
